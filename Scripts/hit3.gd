...
func _ready() -> void:
	$Miss3.visible_characters  = 0
	$Good3.visible_characters  = 0
	$Perfect3.visible_characters  = 0
  

# processes collsion shapes overlaps
func _process(delta: float) -> void:
	if Input.is_action_just_pressed("Hit 3") and get_tree().get_root().get_child_count()> 6:
		get_tree().get_root().get_child(4).free()

	if (PerfectHit ==true) and Input.is_action_just_pressed("Hit 3"): 
		PHit1 = true
		#print("PERFECT")
		score += 3
		#print(score)
		$Perfect3.visible_characters = -1
		$TimerP3.start(0.3)
		PerfectHit = false
	...
	if (nh1 ==true) and Input.is_action_just_pressed("Hit 3") and(PerfectHit !=true):
		GHit1 = true
		#print("GOOD")
		score += 1
		#print(score)
		$Good3.visible_characters = -1
		$TimerG3.start(0.3)
		nh1 = false
